...
angular.module('springBootAdmin')
  	...
  	.controller('detailsConfigpropsCtrl', ['$scope', 'application', 'ApplicationDetails', 
  			function ($scope, application, ApplicationDetails) {
  		$scope.application = application;
  		$scope.typeOf = function(input) {
  		    return angular.isObject(input) ? typeof input : '';
  		}
  		ApplicationDetails.getConfigprops(application).success(function(configprops) {
			$scope.configprops = configprops;
		}).error( function(error) {
			$scope.error = error;
		});
  	}])
